...
BASE_DIR  = os.path.dirname(os.path.abspath(__file__))      # ex05.py 위치
file_score = os.path.join(BASE_DIR, '..', 'data', '학생성적.csv')
file_info  = os.path.join(BASE_DIR, '..', 'data', '학생정보.csv')

# 처음엔 존재 확인(디버그용)
print("score path:", os.path.abspath(file_score), os.path.exists(file_score))
print("info  path:", os.path.abspath(file_info),  os.path.exists(file_info))

# 읽을 때 인코딩 명시
score = pd.read_csv(file_score, index_col='지원번호', encoding='utf-8-sig')
info  = pd.read_csv(file_info,  index_col='지원번호', encoding='utf-8-sig')
