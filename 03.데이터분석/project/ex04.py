import os
import pandas as pd
from ex05 import submenu

# ── 경로: 이 파일 기준으로 data/학생정보.csv를 가리키도록 (OS 무관)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
file_name = os.path.join(BASE_DIR, '..', 'data', '학생정보.csv')

# ── 최초 실행 시 파일이 없으면 빈 CSV 생성해서 FileNotFound 방지
if not os.path.exists(file_name):
    os.makedirs(os.path.dirname(file_name), exist_ok=True)
    df_init = pd.DataFrame(columns=['이름', '학교', '키', 'SW특기'])
    df_init.index.name = '지원번호'
    df_init.to_csv(file_name, encoding='utf-8-sig')
