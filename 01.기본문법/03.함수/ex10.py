...
while True:
    ...
    if menu =="0":
        ...
        if idx == None:
            print(f'{code}번 상품이 존재하지 않습니다.')
        else:
            s = sale[idx]
            print(f"상품이름:{s['name']}")
            print(f"상품가격:{s['price']:,}만원")
            print(f"상품수량:{s['qnt']:,}개") 
            print(f"총매출:{s['price'] * s['qnt']:,}만원")  
               
    elif menu =="3": #목록
        printHeader()
        for s in sale:   # 리스트 전체 출력
            printItem(s)

    elif menu =="4": #삭제
        code = inputNum("삭제코드>")
        idx = search(sale,code)    # ❗ idx 먼저 구해야 함(기존 코드에선 미정의 사용)
        if idx == None:
            print(f"{code}번이 없습니다")
        else:
            s = sale[idx]
            print(s)
            sel = input("삭제하실래요?(Y)")
